...
@click.option('--task', '-t', required=True,
              help='Task description (e.g., "http client", "jwt parser")')
...
@click.option('--category', '-c',
              help='Package category for file prioritization')
@click.option('--json', 'output_json', is_flag=True,
              help='Output as JSON')
@click.option('--verbose', '-v', is_flag=True,
              help='Enable verbose logging')
def ingest(repo_url: str, language: Optional[str], category: Optional[str],
...
def cache_info() -> None:
    ...
    try:
        ...
        cursor = conn.execute("SELECT COUNT(*) FROM package_signals")
