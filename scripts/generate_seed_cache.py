...
async def generate_seed_cache(output_path: Path, limit_per_lang: int = 30) -> None:
    ...
    SQLiteCache(output_path.parent)
    total_cached = 0
    failed = []

    for language, _package_names in SEED_PACKAGES.items():
    ...
    # Generate summary
    logger.info(f"\n{'=' * 60}")
    logger.info("Seed cache generation complete!")
...
def verify_seed_cache(cache_path: Path) -> bool:
    ...
    if actual_checksum == expected_checksum:
        logger.info("✓ Seed cache verification passed")
        logger.info(f"  SHA256: {actual_checksum}")
        return True
    else:
        logger.error("✗ Seed cache verification FAILED")
...
async def main():
    ...
    parser.add_argument(
        "--limit", type=int, default=30, help="Maximum packages to cache per language"
    )
    parser.add_argument(
        "--verify", action="store_true", help="Verify existing seed cache instead of generating"
    )
