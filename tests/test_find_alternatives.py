...
def test_find_alternatives_success(
    mock_collect, mock_get_client, mock_mapper_cls, mock_cache_cls, mock_load_config,
    mock_config, sample_package_data
):
    ...
    client_instance.search.return_value = [
        PackageCandidate(name='requests', registry='pypi', description='HTTP',
                         version='2.31.0', github_url='https://github.com/psf/requests',
                         weekly_downloads=45_000_000),
    ]
...
@patch('priorart.core.find_alternatives.SQLiteCache')
@patch('priorart.core.find_alternatives.QueryMapper')
def test_find_alternatives_no_taxonomy_match(mock_mapper_cls, mock_cache_cls, mock_load_config, mock_config):
...
def test_find_alternatives_below_threshold(
    ...
    client_instance = MagicMock()
    client_instance.search.return_value = [
        PackageCandidate(name='tiny-pkg', registry='pypi', description='tiny',
                         version='0.1.0', weekly_downloads=10),
    ]
    client_ctx.__enter__ = MagicMock(return_value=client_instance)
    client_ctx.__exit__ = MagicMock(return_value=False)
    mock_get_client.return_value = client_ctx
    ...
    mock_collect.return_value = {
        'name': 'tiny-pkg', 'package_name': 'tiny-pkg', 'registry': 'pypi',
        'weekly_downloads': 10, 'star_count': 5,
        'language': 'python',
    }
...
def test_collect_signals_no_github_url(mock_config):
    ...
    candidate = PackageCandidate(
        name='obscure', registry='pypi', description='no github',
        version='1.0.0', github_url=None,
    )
    cache = MagicMock()
...
def test_find_alternatives_explain_mode(
    ...
    assert all(k in bd for k in ('reliability', 'adoption', 'versioning', 'activity_regularity', 'dependency_health'))
...
def test_fetch_fresh_signals_with_github(mock_candidate, mock_config):
    ...
    gh_signals = GitHubSignals(
        star_count=50000, fork_count=9000, open_issues_count=250,
        mttr_median_days=3.5, mttr_mad=1.2, mttr_state='measured',
        weekly_commit_cv=0.25, recent_committer_count=15,
        days_since_last_commit=5, closed_issues_last_year=800,
    )
...
def test_collect_signals_github_url_from_snapshot(mock_config):
    ...
    candidate = PackageCandidate(
        name='cached-pkg', registry='pypi', description='Has cache',
        version='1.0.0', github_url=None,
    )
    cache = MagicMock()
...
def test_find_alternatives_collect_signals_exception(
    ...
    client_instance = MagicMock()
    client_instance.search.return_value = [
        PackageCandidate(name='pkg', registry='pypi', version='1.0.0',
                         github_url='https://github.com/o/r', weekly_downloads=1000),
    ]
    client_ctx.__enter__ = MagicMock(return_value=client_instance)
    client_ctx.__exit__ = MagicMock(return_value=False)
    mock_get_client.return_value = client_ctx
...
def test_find_alternatives_score_exception(
    ...
    client_instance = MagicMock()
    client_instance.search.return_value = [
        PackageCandidate(name='requests', registry='pypi', version='2.31.0',
                         github_url='https://github.com/psf/requests',
                         weekly_downloads=45_000_000),
    ]
    client_ctx.__enter__ = MagicMock(return_value=client_instance)
